...
async def test_create_embedding_config(
    ...
    result = await db_session.execute(
        ...
        .where(WorkflowEmbeddingConfig.workflow_id == workflow.id)
    )
...
async def test_update_embedding_config(
    ...
    config = WorkflowEmbeddingConfig(
        ...
        enabled=False,
        embedding_template="Simple template"
    )
...
async def test_get_embedding_config(
    ...
    config = WorkflowEmbeddingConfig(
        workflow_id=workflow.id,
        enabled=True,
        embedding_template="Test template"
    )
    db_session.add(config)
    await db_session.commit()
...
@pytest.mark.asyncio
async def test_embedding_config_cascade_delete(
    db_session: AsyncSession
):
