...
class TestWorkflowExecution:
    ...
    async def test_complete_workflow_execution(
        self, db_session: AsyncSession, sample_workflow, sample_request, mock_redis
    ):
        ...
        with patch("httpx.AsyncClient") as mock_client_class:
            ...
            with patch(
                "app.services.embedding_service.embedding_service.generate_embedding"
            ) as mock_embed:
    ...
    async def test_workflow_failure_handling(
        self, db_session: AsyncSession, sample_workflow, sample_request
    ):
    ...
    async def test_concurrent_job_processing(
        ...
        for request in requests:
            job_id = await job_service.create_job(
                request_id=request.id, workflow_id=sample_workflow.id
            )
    ...
    async def test_event_publishing_during_execution(
        ...
        with patch("app.services.event_bus.redis_client", mock_redis):
            ...
            job_id = await job_service.create_job(
                request_id=sample_request.id, workflow_id=sample_workflow.id
            )
    ...
    async def test_embedding_generation_after_workflow(
        ...
        with patch("httpx.AsyncClient") as mock_client_class:
            ...
            with patch(
                "app.services.job_service.JobService._send_to_embedding_service",
                mock_send_embedding,
            ):
                job_service = JobService(db_session)
                ...
                await job_service._generate_workflow_embedding(
                    sample_request.id, sample_workflow.id, db_session
                )
