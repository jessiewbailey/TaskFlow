...
async def test_create_similarity_config(
    ...
    result = await db_session.execute(
        ...
        .where(WorkflowSimilarityConfig.workflow_id == workflow.id)
    )
...
async def test_update_similarity_config(
    ...
    config = WorkflowSimilarityConfig(
        ...
        fields=[{"name": "summary", "source": "block_output"}]
    )
...
@pytest.mark.asyncio
async def test_similarity_config_cascade_delete(
    db_session: AsyncSession
):
    ...
    config = WorkflowSimilarityConfig(
        workflow_id=workflow.id,
        fields=[{"name": "test", "source": "block_output"}]
    )
    db_session.add(config)
    await db_session.commit()
