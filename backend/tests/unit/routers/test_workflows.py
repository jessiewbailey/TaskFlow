...
class TestWorkflowRoutes:
    ...
    async def test_create_workflow_invalid_name(self, async_client: AsyncClient):
        ...
        invalid_data = {"name": "", "description": "Test", "blocks": []}  # Empty name

        response = await async_client.post("/api/workflows", json=invalid_data)
...
class TestWorkflowEmbeddingConfig:
    ...
    async def test_create_embedding_config(
        self, async_client: AsyncClient, sample_workflow_data, sample_embedding_config
    ):
    ...
    async def test_update_embedding_config(
        ...
        response = await async_client.post(
            f"/api/workflows/{workflow_id}/embedding-config", json=updated_config
        )
...
class TestWorkflowSimilarityConfig:
    ...
    async def test_create_similarity_config(
        ...
        similarity_config = {
            "fields": [
                {"name": "Summary", "type": "text", "source": "Summarize.summary"},
                {"name": "Score", "type": "score", "source": "SIMILARITY_SCORE"},
            ]
        }

        response = await async_client.post(
            f"/api/workflows/{workflow_id}/similarity-config", json=similarity_config
        )
