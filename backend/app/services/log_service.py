...
class LogStreamingService:
                        ...
                        for log_entry in new_logs:
                            if websocket in self.active_streams:
                                try:
                                    ...
                                    logger.error("Failed to send new log to WebSocket", error=str(e))
                                    break
                            else:
                                break
    ...
    async def _stream_docker_logs(self, websocket):
        ...
        try:
            ...
            logger.info("Started Docker Ollama log streaming", container=self.ollama_container_name)
        ...
        try:
            ...
            def _list_pods():
                pod_list = self.k8s_client.list_namespaced_pod(
                    namespace=self.namespace,
                    label_selector=self.ollama_pod_selector
                )
                return [pod.metadata.name for pod in pod_list.items if pod.status.phase == "Running"]
