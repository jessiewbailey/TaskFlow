...
from app.models.webhook_models import WebhookEventType, WebhookDeliveryStatus
import structlog
...
class WebhookService:
    ...
    async def _deliver_webhook(self, webhook: Webhook, delivery: WebhookDelivery):
            ...
            try:
                ...
                async with httpx.AsyncClient() as client:
                    ...
                    if response.is_success:
                        ...
                        logger.info(f"Successfully delivered webhook {webhook.id} for event {delivery.event_type}")
                        return
                    else:
        ...
        await self._update_delivery(delivery)
        logger.error(f"Failed to deliver webhook {webhook.id} after {webhook.retry_count + 1} attempts")
    ...
    async def test_webhook(self, webhook: Webhook, event_type: str, sample_data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
