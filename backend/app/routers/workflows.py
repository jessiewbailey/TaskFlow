...
from app.models.schemas import Workflow, WorkflowBlock, WorkflowBlockInput, WorkflowStatus, BlockInputType, BlockType, WorkflowDashboardConfig, DashboardLayout
from app.models.pydantic_models import (
    CreateWorkflowRequest, UpdateWorkflowRequest, WorkflowResponse, 
    WorkflowListResponse, WorkflowBlockResponse, WorkflowBlockInputResponse,
    DashboardConfigRequest, DashboardConfigResponse
)
import structlog
import httpx
    ...
    for block_index, block_data in enumerate(workflow.blocks):
        ...
        for input_data in block_data.inputs:
            try:
                ...
                if input_data.source_block_id is not None and input_data.input_type == 'BLOCK_OUTPUT':
                    if 0 <= input_data.source_block_id < len(created_blocks):
                        source_block_id = created_blocks[input_data.source_block_id].id
                        logger.info(f"Mapped source_block_id {input_data.source_block_id} to database ID {source_block_id}")
                    else:
                        # Invalid block index, skip this input
                        logger.warning(f"Invalid source_block_id {input_data.source_block_id} for block {block_index}")
                        continue
        ...
        for block_index, block_data in enumerate(workflow_update.blocks):
            ...
            for input_data in block_data.inputs:
                ...
                if input_data.source_block_id is not None and input_data.input_type == 'BLOCK_OUTPUT':
                    if 0 <= input_data.source_block_id < len(created_blocks):
                        source_block_id = created_blocks[input_data.source_block_id].id
                        logger.info(f"Update: Mapped source_block_id {input_data.source_block_id} to database ID {source_block_id}")
                    else:
                        # Invalid block index, skip this input
                        logger.warning(f"Update: Invalid source_block_id {input_data.source_block_id} for block {block_index}")
                        continue
