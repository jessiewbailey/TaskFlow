...
from app.models.schemas import Webhook, WebhookDelivery, User
...
async def list_webhooks(
    ...
    return WebhookListResponse(
        webhooks=[WebhookResponse.from_orm(w) for w in webhooks],
        total=len(webhooks)
    )
...
async def list_webhook_deliveries(
    ...
    # Count total
    count_query = select(func.count(WebhookDelivery.id)).where(WebhookDelivery.webhook_id == webhook_id)
    if status:
